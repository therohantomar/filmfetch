        ...
        <div>
            <div id="Carousel_Box" className="flex flex-col items-center bg-movieAlbum w-full justify-center relative transition-all  ">
                <section key={trendingAll[index]?.id} id="Carousel_Block" className="w-full flex p-4  gap-4 h-full justify-center flex-wrap bg-slate-800 opacity-95">
                    ...
                    <div className="flex text-slate-900 bg-gray-400  flex-col absolute  bottom-0 opacity-80  p-2  ">
                    ...
                    <p className="xs:text-sm sm:text-md md:text-lg lg:text-md xl:text-md 2xl:text-md font-bold my-2 w-3/4    overflow-clip">Overview: {trendingAll[index]?.overview}</p>
                    </div>
                </section>
            </div>
        </div>
